...
class ChangePasswordViewModel : ViewModel() {
    ...
    fun sendResetPasswordRequest(context: Context) {
        viewModelScope.launch {
            if (_password.value == _secondpassword.value) {
                try {
                    val response = RetrofitInstance.getApi(context).passwordChange()
                    if (response.isSuccessful) {
                        _changeSuccess.value = true
                        _message.value = "Код для смены отправлен на почту"
                    } else {
                ...
                _message.value = "Пароли не совпадают"
            }
        }
    }
}