...
import com.example.familydocumentsapp.model.PasswordChangeConfirmRequest
...
class ChangePasswordConfirmViewModel : ViewModel() {
    ...
    fun confirmCode(context: Context, newpassword: String) {
        viewModelScope.launch {
            _isLoading.value = true
            _errorMessage.value = ""
            try {
                val response = RetrofitInstance.getApi(context).passwordChangeConfirm (
                    PasswordChangeConfirmRequest(_code.value, newpassword))
