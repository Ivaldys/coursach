...
class ForgotPasswordConfirmViewModel : ViewModel() {
    ...
    fun confirmCode(email: String, context: Context) {
        viewModelScope.launch {
            _isLoading.value = true
            _errorMessage.value = ""
            try {
                val response = RetrofitInstance.getApi(context).passwordRecoveryConfirm (
                    PasswordRecoveryConfirmRequest(email, _code.value, _newpassword.value, _confirmpassword.value))
