...
import com.example.familydocumentsapp.model.CreateFamilyFolderRequest
...
import com.example.familydocumentsapp.model.FamilyFolderById
...
class FamilyHomeViewModel(private val context: Context) : ViewModel() {
    ...
    fun loadFamilyStorage(folderId: Int? = null, familyId: Int) {
            ...
            viewModelScope.launch {
                try {
                    ...
                    val response = RetrofitInstance.getDataApi(context).getFamilyStorage(folderId, familyId)
                    if (response.isSuccessful) {
                        val data = response.body()
                        if (_currentFolderId.value != null){
                            val response2 = RetrofitInstance.getDataApi(context).getFamilyFolderById(folderId!!, familyId)
    ...
    suspend fun uploadScannedDocument(
        ...
        val familyPart = familyId.toRequestBody("text/plain".toMediaTypeOrNull())
        val response = RetrofitInstance.getDataApi(context).uploadFamilyDocument(
            file = body,
            name = namePart,
        ...
        viewModelScope.launch {
            try {
                ...
                val response = withContext(Dispatchers.IO) {
                    RetrofitInstance.getDataApi(context).getFamilyPresignedDownloadUrl(doc.id)
                }
    ...
    fun createFamilyFolder(name: String, parentId: Int? = null, family: Int) {
        viewModelScope.launch {
            try {
                val response = RetrofitInstance.getDataApi(context).createFamilyFolder(
                    CreateFamilyFolderRequest(name,family, parentId))
                if (response.isSuccessful) {
                    loadFamilyStorage(familyId = family)
