...
import com.example.familydocumentsapp.viewmodel.HomeViewModel.SortType
import com.example.familydocumentsapp.viewmodel.HomeViewModel.*
...
fun HomeScreen(
    ...
    LaunchedEffect(Unit) {
        if (isOnline == true) {
            viewModel.loadPersonalStorage()
        }
        if (isOnline == false) {
            viewModel.loadPersonalStorage(onlinemod =false)
        }
            ...
            ModalDrawerSheet {
                Column(
                    ...
                    families.forEach { family ->
                        Button(
                            onClick = { onFamilySelected(family.family_id) },
                            modifier = Modifier
                                .fillMaxWidth()
                                .padding(horizontal = 16.dp),
                            colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF8B0000), contentColor = Color.White),
            ...
            topBar = {
                TopAppBar(
                    ...
                    actions = {
                        IconButton(onClick = {
                            if (isOnline ==true) {
                                viewModel.loadPersonalStorage(viewModel.currentFolderId.value)
                            } else  {
                                viewModel.loadPersonalStorage(onlinemod = false)
                            }
                        }) {
                        ...
                        AlertDialog(
                            ...
                            confirmButton = {
                                TextButton(onClick = {
                                    ...
                                    coroutineScope.launch {
                                        viewModel.createPersonalFolder(
                        ...
                        item {
                            ListItem(
                                ...
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .clickable {
                                        viewModel.loadPersonalStorage(parentId)
                                    }
                            )
                            Divider()
                        }
                            ...
                            ListItem(
                                ...
                                modifier = Modifier
                                    ...
                                    .clickable {
                                        ...
                                        viewModel.loadPersonalStorage(folder.id)
...
fun ModeSwitchRow(
    ...
    Row(
        ...
        Button(
            ...
            colors = ButtonDefaults.buttonColors(
                containerColor = if (isOnline) Color(0xFF8B0000) else MaterialTheme.colorScheme.surfaceVariant
            )
        ) {
            Text("Онлайн режим")
        }
